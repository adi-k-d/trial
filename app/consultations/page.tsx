...
export default function MyConsultationsPage() {
  ...
  useEffect(() => {
    const fetchConsultations = async () => {
      ...
      if (role === 'doctor') {
        // No filters: show all consultations to doctors
      } else if (role === 'admin') {
        // Optionally: show all consultations to admins
      } else {
        // For patients, show only their own consultations
  ...
  <div className="flex items-center justify-between">
    ...
    <Link href="/consultations/new">
      <Button className="bg-[#265c8f] text-white hover:bg-[#1e4b75]">
  ...
  {!loading &&
    consultations?.map((c) => (
      <Card
        key={c.id}
        className="bg-white border border-[#e8dfca] shadow-sm hover:shadow-md transition rounded-xl"
      >
